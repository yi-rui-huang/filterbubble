...
export async function selectMoviesForExperiment(fullMovieDataset, userProfile, userQuery) {
  ...
  const ALL_GENRES = [
      ...
      'Mystery', 'Romance', 'Sci-Fi', 'Sport', 'Thriller', 'War', 'Western',
      'News', 'Reality-TV', 'Talk-Show', 'Game-Show' // 共26种类型 (已移除Short、Superhero和Adult)
  ];
  ...
  const pickNextMovieForGenre = (genre) => {
      ...
      for (const movie of candidates) {
          if (!triedTconsts.has(movie.tconst)) {
              triedTconsts.add(movie.tconst);
              return { ...movie, targetGenre: genre }; // 附加目标类型，方便后续追踪
          }
      }
      return null;
  };
  ...
  if (finalMovies.length >= 12) {
    // 如果有足够的电影，随机选择12部
    const result = shuffleArray(finalMovies).slice(0, 12);
    console.log(`✅ 成功返回12部电影（从${finalMovies.length}部中选择）`);
    return result;
  } else {
    console.warn(`⚠️ 警告：只找到 ${finalMovies.length} 部符合条件的电影，少于目标数量12部。将从候选池中补充电影。`);
    
    // 如果电影不足12部，从候选池中随机选择补充
    const remainingCount = 12 - finalMovies.length;
    
    // 过滤掉已经在finalMovies中的电影
    const usedTconsts = new Set(finalMovies.map(movie => movie.tconst));
    let remainingCandidates = candidatePool.filter(movie => !usedTconsts.has(movie.tconst));
    
    // 如果候选池中的电影不足以补充到12部，则允许重复使用已有电影
    if (remainingCandidates.length < remainingCount) {
      console.warn(`⚠️ 候选池中只有${remainingCandidates.length}部可用电影，不足以补充到12部。将重复使用一些电影。`);
      // 如果候选池完全为空，则使用整个数据集
      if (remainingCandidates.length === 0) {
        remainingCandidates = fullMovieDataset.filter(movie => 
          movie.titleType === 'movie' && 
          movie.isAdult === '0' && 
          parseInt(movie.startYear, 10) >= MIN_YEAR &&
          validateGenres(movie.genres)
        );
        console.log(`使用整个数据集作为候选池，共${remainingCandidates.length}部电影`);
      }
      
      // 如果仍然没有足够的电影，则允许重复使用finalMovies中的电影
      if (remainingCandidates.length < remainingCount) {
        console.warn(`⚠️ 即使使用整个候选池，仍然不足以补充到12部电影。将重复使用已选电影。`);
        remainingCandidates = [...remainingCandidates, ...finalMovies];
      }
    }
    
    // 随机打乱并选择需要的数量
    const shuffledCandidates = shuffleArray(remainingCandidates);
    const additionalMovies = [];
    
    // 确保我们能获取到足够的电影
    for (let i = 0; i < remainingCount; i++) {
      // 如果候选池中的电影不足，则循环使用
      const index = i % shuffledCandidates.length;
      const movie = shuffledCandidates[index];
      additionalMovies.push({
        ...movie,
        targetGenre: 'Supplementary' // 标记为补充电影
      });
    }
    
    console.log(`从候选池中额外选择了 ${additionalMovies.length} 部电影补充到推荐列表中`);
    
    // 合并并随机打乱顺序
    const result = shuffleArray([...finalMovies, ...additionalMovies]);
    
    // 最后确认一下电影数量
    console.log(`最终返回 ${result.length} 部电影`);
