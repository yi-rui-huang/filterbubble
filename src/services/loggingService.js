...
export async function logQuestionnaireResponses(questionnaireId, responses) {
  // 同时保存到localStorage，方便后续访问
  try {
    localStorage.setItem('filterbubble_questionnaire_' + questionnaireId, JSON.stringify(responses));
  } catch (error) {
    console.error('Failed to save questionnaire responses to localStorage:', error);
  }
 ...
 * @param {string} questionnaireId - ID of the questionnaire
 * @returns {Object|null} User's responses or null if not found
 */
export function getQuestionnaireResponses(questionnaireId) {
  try {
    // 首先尝试从localStorage获取
    const localResponses = localStorage.getItem('filterbubble_questionnaire_' + questionnaireId);
    if (localResponses) {
      return JSON.parse(localResponses);
    }
    
    // 如果本地没有，返回null
    // 注意：这里没有从Firebase获取，因为实时需求无法等待网络请求
    // 如果需要从Firebase获取，可以添加异步方法
    return null;
  } catch (error) {
