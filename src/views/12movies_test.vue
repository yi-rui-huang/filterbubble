        ...
        <div class="movies-grid">
          ...
          >
            ...
            <div class="movie-info">
              ...
              <p class="rating" v-if="movie.averageRating">
                评分: <span class="rating-value">{{ movie.averageRating }}</span> 
                ({{ movie.numVotes || 0 }}人评价)
              </p>
              <p class="director" v-if="movie.directors && movie.directors !== 'N/A'">
                导演: {{ movie.directors }}
              </p>
            </div>
          </div>
...
<script>
import { selectMoviesForExperiment } from '../utils/12movies_test.js';
  ...
  methods: {
    loadUserProfile() {
      try {
        ...
        } else if (this.userProfile.in_profile_genres.length < 2) {
          // 如果用户画像中的类型少于2个，添加默认类型确保至少有2个
          const defaultGenres = ['Comedy', 'Drama', 'Action', 'Romance', 'Thriller'];
          const existingGenre = this.userProfile.in_profile_genres[0];
          // 从默认类型中选择一个不同于现有类型的类型
          const additionalGenres = defaultGenres.filter(genre => genre !== existingGenre);
          this.userProfile.in_profile_genres.push(additionalGenres[0]);
          console.warn(`用户画像类型不足2个，已添加默认类型 ${additionalGenres[0]}`);
          console.log('更新后的用户画像:', this.userProfile);
        } else {
    ...
    async loadMovieDataset() {
      try {
        ...
        // 使用正确的公共路径
        const response = await fetch('/title.basics.movie.filtered.tsv');
        if (!response.ok) {
    ...
    async testMovieSelection() {
      ...
      // 如果用户画像不完整，使用默认画像
      if (!this.userProfile || !this.userProfile.in_profile_genres || this.userProfile.in_profile_genres.length < 2) {
        console.log('用户画像数据不完整，使用默认画像');
        this.userProfile = {
          in_profile_genres: ['Drama', 'Comedy', 'Action', 'Romance'],
      ...
      try {
        ...
        // 直接使用筛选结果，不添加评分和导演信息
        this.selectedMovies = result.map(movie => {
          return {
            ...movie,
            // 添加默认值，避免模板引用错误
            averageRating: null,
        ...
        // 确认电影数量
        console.log(`筛选完成，返回 ${this.selectedMovies.length} 部电影`);
