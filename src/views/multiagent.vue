    ...
    <div class="sidebar movie-recommendations-sidebar">
      ...
      <div class="movie-list" ref="movieListContainer">
        ...
        <div 
          ...
          class="movie-card"
          :class="[movie.recommendedByAgents ? '' : `recommended-by-${movie.recommendedBy}`]"
          @click="openImdbPage(movie)"
...
export default {
  ...
  computed: {
    // 过滤出除moderator以外的所有agent
    filteredAgents() {
      const agents = {};
      Object.entries(this.agentProfiles.agents).forEach(([key, agent]) => {
        if (key !== 'moderator') {
          agents[key] = agent;
        }
      });
  ...
  created() {
    ...
    // Temporarily commented out transition page requirement
    /*
    ...
    */
    ...
    // Welcome messages have been removed
    ...
    async sendMessage() {
      ...
      try {
              ...
              selectBestAgent(this.userQuestionnaireResponses, mainAgents)
                .then(preferredAgent => {
                  if (preferredAgent) {
                    ...
                    // 如果响应消息还没有生成，重新排序专家
                    this.reorderAgentsWithPreferred(preferredAgent);
                  }
                })
                .catch(error => {
        ...
        const processAgentResponses = selectedAgents.map(async (agentKey) => {
          console.log(`%c开始处理代理 ${agentKey} 的回复`, 'background: #007; color: #fff; padding: 2px;');
