...
import { generateAgentConversation } from '../utils/response3-1-2.js';
import { generateSecondRoundResponse } from '../utils/response3-2-2.js';
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      console.log('[multiagenttest] Movie dataset loaded successfully, count:', this.movieDataset?.length || 0);
    } catch (error) {
      console.error('[multiagenttest] Failed to load movie dataset in mounted:', error);
    }
  ...
  methods: {
    ...
    mapAgentIdToKey(agentId) {
      // Use the same logic as findAgentKeyByAgentId for consistency
      return this.findAgentKeyByAgentId(agentId);
    },
    ...
    async loadMovieDataset() {
      try {
        ...
        console.error('[multiagenttest] Error details:', {
          message: error.message,
          stack: error.stack,
          name: error.name
        });
        this.movieDataset = []; // Fallback to empty array
        throw error; // Re-throw to be caught by mounted handler
      }
    },
    ...
    async sendMessage() {
      ...
      try {
        ...
        // Log user message to conversation_turns
    ...
    async generateAgentResponses(userQuery) {
      this.isAgentTyping = true;
      
      try {
        console.log(`[multiagenttest] Starting response generation workflow... (Round: ${this.isFirstRound ? 'First' : 'Subsequent'})`);
        ...
        if (this.isFirstRound) {
          ...
          if (!this.movieDataset || this.movieDataset.length === 0) {
            console.warn('[multiagenttest] Movie dataset not loaded, attempting to reload...');
            try {
              await this.loadMovieDataset();
              if (!this.movieDataset || this.movieDataset.length === 0) {
                throw new Error('Movie dataset still not available after reload attempt');
              }
            } catch (reloadError) {
              console.error('[multiagenttest] Failed to reload movie dataset:', reloadError);
              throw new Error('Movie dataset not loaded');
            }
          }
          ...
          await this.saveMovieSetToFirestore(selectedMovies, userProfile, userQuery);
          
          // Step 4: Prepare movie data in the format expected by response3-1-2.js
          const movieData = {
            inProfileMovies: selectedMovies.inProfileMovies || selectedMovies.slice(0, 6),
            outOfProfileMovies: selectedMovies.outOfProfileMovies || selectedMovies.slice(6, 12)
          };
          
          console.log('[multiagenttest] Prepared movie data for response3-1-2:', movieData);
          
          // Step 5: Generate agent conversation using the new single-call architecture
          console.log('[multiagenttest] Generating agent conversation with single-call architecture...');
          ...
          // Check if agentConversation has the new structure with conversation and movie_pitches
          let conversationArray;
          if (agentConversation.conversation && Array.isArray(agentConversation.conversation)) {
            conversationArray = agentConversation.conversation;
            // Process movie pitches if available
            if (agentConversation.movie_pitches && Array.isArray(agentConversation.movie_pitches)) {
              agentConversation.movie_pitches.forEach(pitch => {
                this.moviePitches[pitch.movie_title] = {
                  agentId: pitch.recommending_agent,
                  pitch: pitch.pitch
                };
              });
            }
          } else if (Array.isArray(agentConversation)) {
            // Fallback for old format
            conversationArray = agentConversation;
          } else {
          ...
          for (let i = 0; i < conversationArray.length; i++) {
            const agentResponse = conversationArray[i];
            ...
            // Process movie recommendations from agent responses
            await this.extractMovieRecommendation(agentResponse.dialogue, agentKey || agentResponse.agent_id);
            
            // Log each agent response to conversation_turns
            await this.logConversationToFirestore('agent', agentResponse.dialogue, agentKey || agentResponse.agent_id);
          ...
          this.addMoviesToRecommendations([...movieData.inProfileMovies, ...movieData.outOfProfileMovies]);
    ...
    // 电影推荐相关方法
    // 使用API提取电影推荐
    async extractMovieRecommendation(text, agentKey) {
      try {
...
`;
        ...
        if (responseData && responseData.choices && responseData.choices.length > 0) {
          ...
          try {
            ...
            await this.extractMovieRecommendationFallback(text, agentKey);
          }
        }
      } catch (error) {
    ...
    // 回退方法：使用正则表达式提取电影推荐
    async extractMovieRecommendationFallback(text, agentKey) {
      ...
      for (const title of extractedTitles) {
        if (await this.isValidMovie(title)) {
          const movieData = {
            title: title,
            recommendedBy: agentKey,
            recommendedByAgents: [{ agentType: agentKey, attitude: 'positive' }],
            supportingAgents: [agentKey],
            recommendCount: 1,
            Poster: 'N/A',
            Year: '',
    ...
    async saveMovieSetToFirestore(selectedMovies, userProfile, userQuery) {
      ...
      try {
        if (!this.profileId) {
          console.warn('[MultiAgentTest] No profileId available, skipping movie set save');
          return;
        }
        ...
        const movieSetDoc = {
          ...
          metadata: {
            ...
            roundId: '3',
            experimentType: 'multiagent_conversation',
        ...
        console.log('[MultiAgentTest] Movie set saved to Firestore with ID:', docRef.id);
        console.log('[MultiAgentTest] Saved movie set structure:', {
        ...
        console.error('[MultiAgentTest] Error saving movie set to Firestore:', error);
      }
    },
    ...
    async logConversationToFirestore(sender, text, agentKey = null) {
      ...
      try {
        ...
        const conversationTurn = {
          profileId: this.profileId,
          userId: this.dynamicProfiles?.userId || null,
          roundId: "3", // MultiAgentTest的roundId为3
        ...
        if (sender === 'user') {
          ...
          conversationTurn.agentResponse = {
            ...
            agentName: this.getAgentDisplayName(agentKey) || 'Agent'
          };
        }
    ...
    scrollToAgentMention(movieTitle, agentKey) {
      try {
        if (!movieTitle || !agentKey) return;
        ...
        for (let g = 0; g < this.messageGroups.length; g++) {
          ...
          for (let m = 0; m < group.agentMessages.length; m++) {
            ...
            if ((msg.sender === agentKey) && typeof msg.text === 'string') {
